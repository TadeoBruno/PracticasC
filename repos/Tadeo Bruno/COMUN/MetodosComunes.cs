...
namespace COMUN
{
    static public class MetodosComunes
    {
        ...
        static public int calcularedad(DateTimePicker nacimiento) //Función para calcular edades// by Javier Ramírez//
        {
            int age = DateTime.Now.Year - nacimiento.Value.Year - (DateTime.Now.DayOfYear < nacimiento.Value.DayOfYear ? 1 : 0);
            return age;
        }

        //Formato fecha y Hora//
        public static string formatohorafecha(DateTimePicker fechayhora)
        {
            DateTime Fecha = fechayhora.Value;
            //Fecha corta + tiempo corto
            string hora = string.Format("{0:g}", Fecha);
            return hora;
        }
